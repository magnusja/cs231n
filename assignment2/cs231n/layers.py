...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (mean, var, gamma, beta, x, normalized_x, eps)
...
def batchnorm_backward(dout, cache):
    ...
    mean, var, gamma, beta, x, normalized_x, eps = cache
    ###########################################################################
    ...
    dvar = np.sum((mean - x) / (2 * (var + eps) ** (3/2)) * dnormalized_x, axis=0)
    dx = 1 / np.sqrt(var + eps) * dnormalized_x + dvar * 2.0 / N * (x - mean) + 1.0 / N * dmean
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    new_height = int(1 + (H - pool_height) / stride)
    new_width = int(1 + (W - pool_width) / stride)
    ...
    for i in range(N):
        ...
        for xx in range(0, new_height):
            for yy in range(0, new_width):
                for c in range(C):
                    out[i, c, xx, yy] = np.max(cur_x[c, xx*stride:xx*stride+pool_height, yy*stride:yy*stride+pool_height])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for xx in range(0, new_height):
            for yy in range(0, new_width):
                for c in range(C):
                    cur_x = x[i, c, xx*stride:xx*stride+pool_height, yy*stride:yy*stride+pool_height]
                    max_index = np.argmax(cur_x)
                    mask = np.zeros(cur_x.shape)
                    mask[np.unravel_index(max_index, mask.shape)] = 1
                    dx[i, c, xx*stride:xx*stride+pool_height, yy*stride:yy*stride+pool_height] = mask * dout[i, c, xx, yy]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
